...
class Podcast(db.Model):
    ...
    internal_feed_url = db.Column(db.String(120),nullable=True)
    ...
    def import_string(self, data_string):
        ...
        if (id == self.id):
            ...
            self.style = words[5].decode('utf-8')
            self.stars = int(words[6])
            self.feed_url = words[7].decode('utf-8')
            self.internal_feed_url = words[8].decode('utf-8')
            self.web_url = words[9].decode('utf-8')
...
class Episode(db.Model):
    ...
    audio_time = db.Column(db.String(80),nullable=True)
    ...
    def import_string(self, data_string):
        ...
        if (id == self.id):
            ...
            self.audio_time = words[8].decode('utf-8')
            self.podcast_id = int(words[9])
    
    def audio_size_txt(self):

        audio_size_txt = ''
        if self.audio_size:
            if self.audio_size != 'None':
